...
|---------|------|-------------|
| 4.11.16 | 2026-05-17 | **Agentic validation Phase 11** — end-to-end orchestrator + integration test. New `scripts/run_agentic_validation_demo.py` chains every Phase-1-through-10 component: Stage 1 agentic validation (A→B→C→D, real SDK), Stage 2 legacy bridge (results.json → ValidationReport), Stage 3 revalidation v2 (re-run validation_project on improved bundle), Stage 4 cross-bundle reflection, Stage 5 evolution proposals, Stage 6 source-evolution attempts (dry-run by default, `--apply-evolution` enables real claude_code_edit). Skip flags (`--skip-agentic`) let CI exercise stages 2-6 without burning subscription budget. JSONL progress trail at `/tmp/agentic_demo.jsonl`. New integration test seeds 3 synthetic bundles, runs the full post-validation loop, and verifies every stage's on-disk artifact lands (bridge report.json, revalidation_result.json, tracker JSONLs, agentic_reflection.md, evolution_proposals.jsonl, evolution_attempts.jsonl). Plan v2 of the closed feedback loop is now end-to-end implemented. |
| 4.11.15 | 2026-05-17 | **Agentic validation Phase 10** — source-evolution executor. New `ouroboros/validation/agentic_source_evolution.py` consumes `EvolutionProposal` records and routes them to `claude_code_edit` (the 7-step protocol's edit step) with allow-list + deny-list enforcement. Allow-list: `agentic_prompts/`, `agentic_helpers/`, `agentic_system_prompt.py`, `checks/`. Deny-list: BIBLE.md, safety.py, registry.py, SAFETY.md, sandbox.py — same as the runtime hardcoded sandbox. Multi-stage validation: explicit deny-list match → target_kind/target_path prefix consistency → allow-list prefix → repo-resolve safety → path-traversal check. Dry-run mode (default) produces an attempt record without invoking the SDK; real mode calls `claude_code_edit` via an injectable editor (used by tests to verify the routing without burning subscription budget). Every attempt persists to `knowledge/evolution_attempts.jsonl` for audit. Commit gating remains in the existing agent task pipeline — this module is the typed entry-point, not the commit gate. |
| 4.11.14 | 2026-05-17 | **Agentic validation Phase 9** — methodology evolver retarget. New `EvolutionProposal` dataclass (typed `target_kind` ∈ {prompt, helper, system_prompt, pre_check}) + `AgenticEvolutionProposer` that consumes reflection patterns and emits proposals: `candidate_false_positive` → append directive to `phase_a_methodology.md` telling future sessions to treat the check as a structural artifact; `methodological_motif` → add reusable helper module under `agentic_helpers/` and a Phase B prompt note to PREFER it; `recurring_failure` (with TP confirmation, not also FP-candidate) → append directive making the test mandatory in Phase A. Confidence scales with bundle frequency; proposals ≥ 0.5 persist to `knowledge/evolution_proposals.jsonl`. The proposer never writes source — that's Phase 10's source-evolution agent task running the 7-step protocol against the allow-listed paths. |
| 4.11.13 | 2026-05-17 | **Agentic validation Phase 8** — cross-bundle reflection over agentic artifacts. New `AgenticReflectionEngine` scans every bundle's `results.json` + `methodology.md` and surfaces three classes of pattern: (1) **recurring failures** — check_ids that fail in ≥ 2 bundles (parallel to legacy reflection); (2) **methodological motifs** — test names + metric types that appear in ≥ 3 bundles' methodologies (signal for "promote to default scaffolding in `phase_a_methodology.md` or `agentic_helpers`"); (3) **candidate structural artifacts** — recurring failures with zero `true_positive` records in the EffectivenessTracker (likely structural artifacts like hardcoded Kaggle paths, not real model defects). The third class consumes Phase 7's tracker writes — the closed loop now points at concrete evolution targets. Knowledge digest written to `knowledge/agentic_reflection.md` for the next session's system prompt to read. |
| 4.11.12 | 2026-05-17 | **Agentic validation Phase 7** — EffectivenessTracker writes from agentic revalidation. `AgenticRevalidationPipeline._record_effectiveness()` writes one Signal-A row (`record_recommendation_result`) per applied recommendation with the corresponding per-test before/after metrics namespaced by test id (`quant3.AUC`, etc.) plus the lift. Signal B (`record_finding_feedback`) records `true_positive` (weight 0.5) for fail→pass / warn→pass transitions, `false_positive` (weight 0.3) for pass→fail / pass→warn regressions, and skips ambiguous transitions. Tracker writes are best-effort and never block revalidation success. Cross-bundle aggregated `get_recommendation_stats(check_id)` now answers "which validation findings produce recommendations that ACTUALLY improve real metrics" — the loop signal the methodology evolver will key off in Phase 9. |
| 4.11.11 | 2026-05-17 | **Agentic validation Phase 6** — real lift measurement. New `AgenticRevalidationPipeline` re-runs THE SAME Phase B `validation_project/` on the improved bundle. Workdir built under `improvement/revalidation_workdir/` with symlinks (raw/data + methodology untouched, raw/model_code overridden to improvement/implementation). Subprocesses `python validation_project/run_all.py --tests all --output ./results/results.json` with a 1500s wall-clock cap. Computes per-test verdict transitions (`categorical_lift` — fail_to_pass, warn_to_pass, pass_to_fail, etc.) AND per-metric numerical deltas (`per_test_deltas`, plus flattened `metric_deltas`). Aggregate `improvement_lift` is the sum of (mean proportional metric lift) + (0.2 × net categorical wins-minus-losses). Verdicts: any (fail/warn) → pass = improvement; pass → (fail/warn) = regression; both directions = `mixed`. `RevalidationResult` gained `categorical_lift`, `per_test_deltas`, `source` fields (defaults preserve v1 callers). Execution log persisted alongside results for auditability. |
| 4.11.10 | 2026-05-17 | **Agentic validation Phase 5** — legacy bridge. New `ouroboros/validation/agentic_results_parser.py` translates the agentic `results.json` (schema_version="1") into a legacy `ValidationReport` so improver / effectiveness tracker / reflection engine keep working unchanged. Mappings: qualitative-block tests → `QUAL` pseudo-stage, quantitative-block tests → `QUANT`; `pass` → `severity=pass / passed=True`, `warn` → `warning`, `fail` → `critical` and added to `critical_findings`, `error/deferred` → `info`; one `hard_recommendation` per failed test (evidence string becomes the body); soft findings extracted from `interpretation.md`'s "Soft findings" section; numeric metrics aggregated into `meta_scores`; overall verdict derived from summary counts (any fail → rejected, any warn/error → conditional, all pass → approved). Parser also writes a canonical `results/report.json` alongside the agentic artifacts so reflection only reads one format. |
| 4.11.9 | 2026-05-17 | **Agentic validation Phase 4** — Phase D (report prettification) wired into the agentic runner. `run_phase_d()` drives a minimal SDK session (Read + Write only, no Bash, no Edit) that consumes `results.json` + `interpretation.md` + `methodology.md` and emits `results/report.md` with: overall verdict / hard findings / soft findings / quantitative results table / qualitative findings / errored-deferred / ambiguities. Strict no-new-information constraint — every claim must trace back to results.json or a verbatim quote from interpretation.md / methodology.md. `AgenticValidator.run()` now chains A→B→C→D end-to-end (per-phase failure halts the chain). New `phase_d_report.md` user prompt template (evolution target). The four-phase agentic flow per Plan v2 is now end-to-end wired; integration with the legacy ValidationReport bridge lands in sub-phase 5. |
| 4.11.8 | 2026-05-17 | **Agentic validation Phase 3** — Phase C (execution + interpretation) wired into the agentic runner. `run_phase_c()` drives a new SDK session with `Bash` enabled; Claude Code runs `cd methodology/validation_project && python run_all.py --tests all --output ../../results/results.json`, reads the JSON, then writes `./results/interpretation.md` covering verdict / real issues / clean passes / errored tests / ambiguities. The runner's expected-output gate fails the phase if either `results.json` or `interpretation.md` is missing — Phase C is interpretation, not just execution. `AgenticValidator.run()` chains A→B→C, halting on first-phase failure. New `phase_c_execute.md` user prompt template (evolution target) specifies the 1200s wall-clock cap and the interpretation contract. |
| 4.11.7 | 2026-05-17 | **Agentic validation Phase 2** — Phase B (project authoring) wired into the agentic runner. `run_phase_b()` drives a new SDK session with `Bash` + `Write` + `Edit` enabled; Claude Code reads the Phase-A methodology and authors a full `validation_project/` (one module per qualitative/quantitative test, `run_all.py` runner, `requirements.txt`). Runner verifies expected outputs exist post-session. `AgenticValidator.run()` now chains A→B sequentially and halts on first-phase failure so we don't waste budget on downstream work without upstream inputs. New `phase_b_implement.md` user prompt specifies the project layout contract, the `results.json` schema, and the import-verification step. New `phase_b_model` config (env `OUROBOROS_VALIDATION_PHASE_B_MODEL`, defaults to `opus`) so heavy Phase-B traffic can be routed to Sonnet under rate-limit pressure. |
| 4.11.6 | 2026-05-17 | **Agentic validation Phase 1** (plan v2). New `ouroboros/validation/agentic_runner.py` drives a per-bundle `ClaudeSDKClient` session through the four phases (A: methodology / B: project / C: execute / D: report). Sub-phase 1 wires Phase A end-to-end: the rich system prompt (`agentic_system_prompt.py`) inlines BIBLE.md, the validator-role section of SYSTEM.md, CHECKLISTS.md, accumulated cross-bundle patterns (`validation_patterns.md`), per-model-type knowledge, and the fast deterministic pre-check summary. New types `AgenticPhaseResult` / `AgenticValidationResult` with `to_dict`/`from_dict` for full audit-trail persistence (every assistant message logged to `_agentic_transcripts/phase_{a,b,c,d}.jsonl`). Bundle-scoped PreToolUse path guard inlined into the runner (deny writes outside cwd / to SAFETY-CRITICAL files). Phase-A user prompt template lives at `ouroboros/validation/agentic_prompts/phase_a_methodology.md` — an evolution target for the methodology evolver. |
| 4.11.5 | 2026-05-17 | Improvement-cycle fixes: `model_improver` now reads/writes `.ipynb` notebooks (most Kaggle bundles arrive as notebooks); regression gate is syntax-only after jupyter-magic stripping (the hermetic sandbox lacks numpy/etc and was failing every modified kernel for the wrong reason); `scripts/run_improvement_cycle.py` now awaits `improver.implement()` directly instead of wrapping the coroutine in `run_in_executor`. |
| 4.11.0 | 2026-05-17 | **OAuth subscription migration** (BIBLE v5.1). The sole cloud LLM backend is now the Anthropic subscription via `claude-agent-sdk` + `CLAUDE_CODE_OAUTH_TOKEN`. Per-token providers (OpenRouter, direct OpenAI, direct Anthropic, Cloud.ru, openai-compatible) removed from `ouroboros/llm.py`. New `ouroboros/gateways/claude_code_chat.py` implements `chat()`/`chat_async()`/`vision_query()`/`web_search()` against the SDK with tool-call emulation via prompt injection. `ouroboros/llm.py` shrinks to a thin facade; local llama-cpp path preserved. Multi-model pre-commit review collapses to single-model (constitutional amendment in BIBLE.md §Review and CLAUDE.md "Ten mechanisms" #6); the `multi_model_review` tool name is kept for backwards compatibility but only runs the first listed model. `web_search` migrated from OpenAI Responses API to Claude built-in WebSearch via SDK. Subscription billing returns `cost=0` to the agent's budget; the SDK's `total_cost_usd` is recorded as `notional_cost` for observability. Rate-limit 429 surfaces as a task error and pauses autonomous evolution. `tools/shell.py` `claude_code_edit` and `tools/claude_advisory_review.py` `advisory_pre_review` accept either `CLAUDE_CODE_OAUTH_TOKEN` or `ANTHROPIC_API_KEY` and unset the API key for child processes so the CLI prefers subscription billing. `model_catalog_api.py` returns a curated Anthropic-only static catalog. Onboarding wizard validates an OAuth token (or legacy API key) as the cloud auth source. New `claude_code_chat` adapter test, updated provider-routing/pricing/onboarding/review tests, dropped tests for removed code paths. |
